...
module finality where
  finality : ∀{Δ Σ' Γ E e r k τ} →
               ...
               E ⊢ e ⇒ r ⊣ k →
               r final
  finality E-final _ _ EFun = FVal (VLam E-final)
  finality E-final _ _ EFix = FVal (VFix E-final)
  finality (EF E E-final) _ _ (EVar h2) = E-final h2
  ...
  ... | TALam ctxcons-Ef (TALam _ ta-ef) =
    finality (EF _ new-Ef-final) (EnvInd ctxcons-Ef (preservation ctxcons ta-arg eval-arg)) ta-ef eval-ef
    where
      ...
      ... | FVal (VLam Ef-final) with Ef-final
      ... | EF _ Ef-fin = Ef-fin x'∈Ef
  ...
  ... | TAFix ctxcons-Ef (TAFix _ _ ta-ef) =
    finality (EF _ new-Ef+-final) new-ctxcons ta-ef eval-ef
    where
      new-ctxcons =
      ...
      new-Ef-final  {x'} {rx'} h with ctx-split {Γ = Ef} h
      new-Ef-final  {x'} {rx'} h | Inl x'∈Ef with finality E-final ctxcons ta-f eval-f
      ... | FVal (VFix Ef-final) with Ef-final
      ... | EF _ Ef-fin = Ef-fin x'∈Ef
  ...
  ...  | FVal (VTpl all-fin) = FVal (all-fin h)
  ...
  finality {Σ' = Σ'} (EF _ e-fin) ctxcons (TACase d∈Σ'1 ta h1 h2) (EMatch {E = E} {xj = xj} {r' = r'} i<∥rules∥ form eval eval-ej)
  ...
  ...  | TACtor {cctx = cctx} d∈Σ' Cj∈cctx ta-r'
    rewrite ctxunicity {Γ = π1 Σ'} d∈Σ'1 d∈Σ' | ctxunicity {Γ = cctx} Cj∈cctx1 Cj∈cctx =
      finality (EF _ new-E-final) (EnvInd ctxcons ta-r') ta-ej eval-ej
      where
        new-E-final : ∀{x' rx'} → (x' , rx') ∈ (E ,, (xj , r')) → rx' final
        new-E-final {x'} {rx'} x'∈E+ with ctx-split {Γ = E} x'∈E+
        ... | Inl x'∈E = e-fin x'∈E
        ... | Inr (x'#E , _ , rx'==r') rewrite rx'==r' with finality (EF E e-fin) ctxcons ta eval
        ... | FVal (VCon r'-val) = FVal r'-val
        ... | FCon r'-fin = r'-fin
  finality E-final ctxcons (TACase h ta h2 h3) (EMatchUnfinished eval h4) = FCase (finality E-final ctxcons ta eval) h4 E-final
  finality E-final _ _ EHole = FHole E-final

  {- TODO
  unfinished-has-holes : ∀{r} →
                           r final →
                           (r value → ⊥) →
                           has-hole r
  unfinished-has-holes = ?

  unfinished-has-holes : ∀{Δ Σ' e r k τ} →
                           Δ , Σ' , ∅ ⊢ e :: τ →
                           ∅ ⊢ e ⇒ r ⊣ k →
                           (r value → ⊥) →
                           Σ[ u ∈ Nat ] (hole-name-new e u → ⊥)
  unfinished-has-holes = {!!}
  -}
